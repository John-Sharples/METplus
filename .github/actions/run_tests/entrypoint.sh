...
if [[ "$INPUT_CATEGORIES" == pytests* ]]; then
  ...
  command+="/usr/local/conda/envs/${METPLUS_ENV_TAG}/bin/pytest internal/tests/pytests -q -rEf --junitxml=pytest.xml --cov=metplus --cov-append --cov-report=term-missing:skip-covered | tee pytest-coverage.txt;"
